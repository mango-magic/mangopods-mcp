# MangoPods MCP Founder Beta Application

Live form:

https://mangomagic.live/waitlist?ref=github

Applications now flow through the in-app private beta workflow and the admin beta applications view. Do not publish legacy form or sheet URLs in public repo docs.

The public page should include a no-login demo mode. Real MCP access should be gated through this application.

## Positioning

- MangoPods is not public yet.
- The public site shows the workflow shape without requiring a login.
- Founder beta access is for people with real workflows, reach, and July testing capacity.
- The application should help us prioritise the highest-reach use cases first.

## Recommended Fields

Required:

- Full name
- Email address
- LinkedIn profile URL
- Company, product, community, or project name
- Which best describes you?
  - Vibe coder / independent builder
  - Agency / studio
  - Creator / podcast operator
  - SaaS / product team
  - Internal marketing or enablement team
  - Partner app / marketplace
  - Enterprise media or demand team
  - Other
- What do you want to build with MangoPods MCP?
- Which first workflow would you test?
  - Podcast episode launch pack
  - Pod idea generator to carousel and teleprompter guide
  - Webinar to pipeline pack
  - Customer call to proof page
  - Video clip farm
  - Website or landing page generation
  - LinkedIn content and targeted boost workflow
  - Agency client content sprint
  - Partner app / credit resale workflow
  - White-label creative job workflow
  - Other
- Which MangoPods capabilities do you need first?
  - Podcast idea generation
  - Podcast or video editing
  - Short-form clips and captions
  - Carousels and social creative
  - Teleprompter guides and scripts
  - Website or landing page creation
  - LinkedIn posts and targeted boosts
  - Approval workflows
  - Credit caps and usage reporting
  - White-label or resale workflows
- Write one prompt you would want your agent to run on day one
- Who would this reach?
- How many users, clients, subscribers, followers, or internal teammates could this reach in the next 90 days?
- Where could you share or ship the beta story if it works?
  - LinkedIn
  - Product Hunt
  - Hacker News / Show HN
  - Reddit
  - Indie Hackers
  - Newsletter
  - Client base
  - Internal company rollout
  - App marketplace
  - I would rather stay private
- Can you test in July?
- Do you already have a source asset or workflow ready to test?
- How do you expect to use credits?
  - Use MangoPods credits directly
  - Bring my own MangoPods workspace
  - Bundle credits inside my product
  - Resell fixed creative jobs
  - Agency or studio margin model
  - Internal team usage
  - Not sure yet
- What monthly usage feels realistic if the beta works?
  - A few test jobs per month
  - 10-50 creative jobs per month
  - 51-250 creative jobs per month
  - 251-1,000 creative jobs per month
  - 1,000+ creative jobs per month
  - Not sure yet
- Are you interested in embedding, reselling, or white-labelling MangoPods workflows?
- Would you be open to sharing a public case study if it works?
- Beta access acknowledgement

Optional:

- Website, product, or community URL
- Current tools you use for podcast, video, website, content, or campaign production
- Optional source asset, demo, podcast, website, or example URL
- Anything else we should know

## Live Setup Notes

- The old basic waitlist form has been rebuilt in place as the founder beta application, so the public URL did not change.
- The response spreadsheet has a `Triage Rubric` tab with the 100-point scoring model below.
- Old Google Forms response columns from the previous basic waitlist cannot be deleted because Google locks linked form columns. They are hidden in the live response sheet so the visible intake table starts with the current beta fields.
- `scripts/create-google-form.gs` now targets the existing live form ID. It should only be run when intentionally rebuilding the live application.

## Scoring Rubric

Score each application out of 100.

| Category | Points | What to look for |
| --- | ---: | --- |
| Reach | 25 | Audience, customers, clients, community, distribution, or internal user base |
| Use-case quality | 25 | Clear workflow with obvious MangoPods fit and strong outcome |
| Ability to ship | 20 | Can test in July, has technical owner, has source assets |
| Revenue / credit potential | 15 | Likely recurring credits, resale, bundled workflows, or agency margin |
| Proof value | 10 | Would make a strong public story, demo, or launch example |
| Feedback quality | 5 | Thoughtful applicant likely to help shape the product |

## Access Tiers

- Tier 1 - Founder access: 80-100 points. High reach, strong workflow, July testing capacity, clear proof value.
- Tier 2 - Builder access: 60-79 points. Strong technical or product use case, but smaller reach or less immediate distribution.
- Tier 3 - Partner watchlist: 40-59 points. Interesting agency, studio, or partner model that needs more qualification.
- Tier 4 - Later waitlist: under 40 points. Curious but not yet ready to test or distribute.

## Follow-Up Routing

- Vibe coders: ask what agent/app they want to connect first and what source asset they will test.
- Agencies: ask what repeatable client workflow they want to sell and how they price it today.
- Product teams: ask where MangoPods would sit in the product journey and what action users would click.
- Creators and studios: ask what back catalogue, podcast, webinar, or show workflow they can test in July.
- Partner apps: ask whether they want BYO credits, bundled credits, resale, or white-label creative jobs.

## Manual Follow-Up Template

Subject: MangoPods MCP founder beta

Thanks for applying. The workflow that jumps out is:

`[specific workflow]`

We are prioritising beta access by reach, use-case quality, ability to test in July, credit potential, and proof value. Can you send one source asset, example project, or more detail on how this would sit inside your workflow?

If it is a strong fit, we will route you into the founder beta cohort.
